...
void SimulationConfig::validate() const
{
    ...
    {
        throw std::invalid_argument{
            "Simulation time step cannot exceed total duration"
        };
    }
    ...
    {
        throw std::invalid_argument{
            "Simulation maximum vehicles must be greater than zero"
        };
    }
    ...
    if (
        !std::isfinite(minimumFollowingDistanceMeters) ||
        minimumFollowingDistanceMeters < 0.0
    )
    {
        throw std::invalid_argument{
            "Minimum following distance must be finite and non-negative"
        };
    }
    ...
    {
        throw std::invalid_argument{
            "Reaction time must be finite and positive"
        };
    }
}
